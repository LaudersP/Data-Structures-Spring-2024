...
namespace ssuds {
	...
	public:
		...
		void breadthFirst(ssuds::ArrayListV2<std::pair<N, N>>& traversalMap, N startNode) {
			...
			while (!frontier.empty()) {
				...
				if (it != mData.end()) {
					...
					for (std::pair<N, E>& edges : (*it).second) {
						...
						// Check if that node has been visited or not
						if (!visited[neighbor]) {
		...
		/// <param name="traversalMap"></param>
		/// <param name="startNode"></param>
		void depthFirst(ssuds::ArrayListV2<std::pair<N, N>>& traversalMap, N startNode) {
			// Check that startNode is valid
			if (!contains_node(startNode))
				throw std::out_of_range("ERROR: Start node does not exist in the graph!\n");
