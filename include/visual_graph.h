...
namespace misc
{
	...
	protected:
		...
		/// </summary>
		ssuds::ArrayListV2<int> _selectedNodes;
	...
	public:
		...
		ssuds::ArrayListV2<int> handleMouseClick(const float X, const float Y, bool end = false);

		/// <summary>
		/// Calls the Breath First traversal to the internal graph of nodes
		/// </summary>
		/// <param name="traversalMap"></param>
		void breadthFirstTraversal(ssuds::ArrayListV2<std::pair<int, int>>& traversalMap);

		/// <summary>
		/// Calls the Depth First traversal to the internal graph of nodes
		/// </summary>
		/// <param name="traversalMap"></param>
		void depthFirstTraversal(ssuds::ArrayListV2<std::pair<int, int>>& traversalMap);
	};
}
